...
def print_out(user_card,house_card,winer):
    ...
    print(f"House cards are {house_card}, current score : {sum(house_card)}")
...
def play_bj():
    ...
    if sum(user_card)==21:
        ...
        print_out(user_card,house_card,"House")
    elif sum(user_card) > 21:
        print_out(user_card,house_card,"House")
    else:
        print(f"Your cards are {user_card}, current score : {sum(user_card)}")
        print(f"Computer firts card {house_card[0]}")
        while new_card:
...
while play:
    ...
    query_begin = input("Do you want to play a game of Black Yack? Type 'y' or 'n': ")
    if query_begin == 'y':
        play_bj()
    else :
        print("GoodBye !!!")
